...
void addLinkVideo() {
  GstElement *ximagesrc, *videoscale, *videoconvert, *x264enc, *h264parse,
      *rtph264pay, *udpsink1, *capsfilter1, *capsfilter2;
  GstCaps *caps1, *caps2;
  ...
  if (!data.pipeline || !h264parse || !ximagesrc || !capsfilter1 ||
      !capsfilter2 || !videoscale || !videoconvert || !x264enc || !rtph264pay ||
      !udpsink1) {
  ...
  caps2 = gst_caps_new_simple("video/x-h264", "profile", G_TYPE_STRING, "main",
                              "width", G_TYPE_INT, 1024, "height", G_TYPE_INT,
                              600, NULL);
  ...
  gst_bin_add_many(GST_BIN(data.pipeline), ximagesrc, capsfilter1, videoscale,
                   videoconvert, x264enc, capsfilter2, h264parse, rtph264pay,
                   udpsink1, NULL);

  if (gst_element_link_many(ximagesrc, capsfilter1, videoscale, videoconvert,
                            x264enc, capsfilter2, h264parse, rtph264pay,
                            udpsink1, NULL) != TRUE) {
  ...
  g_object_set(udpsink1, "sync", FALSE, "host", "127.0.0.1", "port", 5001,
               NULL);
