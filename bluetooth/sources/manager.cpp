...
Manager::Manager(QWidget *parent) : QWidget(parent) {
  ...
  connect(/* твоя кнопка */, &QPushButton::clicked, this,
          &Manager::onConnectClicked);
  connect(/* твоя кнопка */, &QPushButton::clicked, this,
          &Manager::onSendStarted);
  ...
  connect(server,
          QOverload<const QString &>::of(&Server::sendClientDisconnected), this,
          QOverload<const QString &>::of(&Manager::onClientDisconnected));
  connect(server, &Server::sendMessageReceived, this, &Manager::onShowMessage);
...
void Manager::onReactOnSocketError(const QString &error) { qDebug() << error; }
...
void Manager::onConnectClicked() {
  const QBluetoothAddress adapter =
      local_adapters.isEmpty()
          ? QBluetoothAddress()
          : local_adapters.at(current_adapter_index).address();
  ...
  if (remote_selector.exec() == QDialog::Accepted) {
    ...
    qDebug() << "Connecting to service 2" << service.serviceName() << "on"
             << service.device().name();
    ...
    connect(client, &Client::sendMessageReceived, this,
            &Manager::onShowMessage);
    connect(client, &Client::sendDisconnected, this,
            QOverload<>::of(&Manager::onClientDisconnected));
    connect(client, QOverload<const QString &>::of(&Client::sendConnected),
            this, &Manager::onConnected);
    connect(client, &Client::sendSocketErrorOccurred, this,
            &Manager::onReactOnSocketError);
