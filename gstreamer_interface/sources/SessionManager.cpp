...
void SessionManager::onKillAudioSession(const QHostAddress ip_address) {
        /* QHash<QPair<QHostAddress, QString>, std::shared_ptr<Session>>::iterator i;
        for (i = live_sessions.begin(); i != live_sessions.end(); ++i) {
            QString key = QString("%1:%2").arg(i.key().first.toString()).arg(i.key().second);
...
void SessionManager::onSetVolume(const QHostAddress ip_address, const float volume)
{
        QString key = "ReciverAudio";
        if (live_sessions.contains(qMakePair(ip_address, key))) {
                auto it = live_sessions[qMakePair(ip_address, key)];
                ...
                /*QMetaObject::invokeMethod(it.get(),
                                          "onVolumeChanged",
                                          Qt::QueuedConnection,
                                          Q_ARG(float, volume));*/
        }
